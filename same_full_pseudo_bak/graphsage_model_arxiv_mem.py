...
class SAGEConv(nn.Module):
	...
	def forward(self, graph, feat):
		...
		see_memory_usage("----------------------------------------after msg_fn = fn.copy_src('h', 'm')")
		h_self = feat_dst
		if self._aggre_type == 'mean':
			graph.srcdata['h'] =  feat_src
			see_memory_usage("----------------------------------------before graph.update_all(fn.copy_src('h', 'm'), fn.mean('m','neigh'))")
			graph.update_all(msg_fn, fn.mean('m', 'neigh'))
			see_memory_usage("----------------------------------------after graph.update_all")
			h_neigh = graph.dstdata['neigh']
			see_memory_usage("----------------------------------------h_neigh = graph.dstdata['neigh']")
			h_neigh = self.fc_neigh(h_neigh)
			see_memory_usage("----------------------------------------after h_neigh = nn.Linear(h_neigh, out_feats, bias=False)")
		# graph.srcdata['h'] = feat_src
...
class GraphSAGE(nn.Module):
	def __init__(self,
		...
		if num_layers==1:
			...
			self.bns.append(nn.BatchNorm1d(out_feats))
		else:
			# input layer
	...
	def forward(self, blocks, x):
		if self.num_layers==1:
			...
			# for i, layer in enumerate(self.layers[:-1]):
				...
				x = self.bns[i](x)
				see_memory_usage("----------------------------------------after model layer "+str(i)+" x = self.bns[i](x)")
				print(x.shape)
